...
class Img2Vec:
    ...
    def similar_images(self, target_file, sim_thr, n=None):
        """
        ...
        target_vector = self.embed_image(target_file) #resnet50 => 1*2048
        ...
        for k, v in self.dataset.items():
            ...
            vector = flat_tensor.detach().numpy()
        ...
        tsne_features = tsne.fit_transform(np.array(vector_arr))
        ...
        over_thr_lst = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if sim_dict[k] > sim_thr]
        heatmap_d = {}
        over_90 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if sim_dict[k] >= 0.9]
        from80_90 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.9 > sim_dict[k] >= 0.8]
        from70_80 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.8 > sim_dict[k] >= 0.7]
        from60_70 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.7 > sim_dict[k] >= 0.6]
        from50_60 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.6 > sim_dict[k] >= 0.5]
        from40_50 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.5 > sim_dict[k] >= 0.4]
        from30_40 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.4 > sim_dict[k] >= 0.3]
        from20_30 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.3 > sim_dict[k] >= 0.2]
        from10_20 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.2 > sim_dict[k] >= 0.1]
        low_10 = [int(k.split('/')[1].split('.')[0]) for k in sim_dict if 0.1 > sim_dict[k]]
        heatmap_d['90'] = over_90
        heatmap_d['80'] = from80_90
        heatmap_d['70'] = from70_80
        heatmap_d['60'] = from60_70
        heatmap_d['50'] = from50_60
        heatmap_d['40'] = from40_50
        heatmap_d['30'] = from30_40
        heatmap_d['20'] = from20_30
        heatmap_d['10'] = from10_20
        heatmap_d['0'] = low_10
